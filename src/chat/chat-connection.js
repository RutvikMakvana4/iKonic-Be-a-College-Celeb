...
import { sendNotificationToSingleUser } from "../../src/common/helper";
...
io.on("connection", async (socket) => {
  ...
  socket.on("sendMessage", async (data) => {
    ...
    if (!userConnected.length) {
      ...
      if (senderData) {
        await sendNotificationToSingleUser(data.receiverId, {
          title: `New message from ${senderData.firstName} ${senderData.lastName}.`,
          body: `New message from ${senderData.firstName} ${senderData.lastName}.`,
          data: {
